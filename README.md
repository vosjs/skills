...
| [`vos-create`](skills/vos-create/SKILL.md) | Create vos programs in the user's own style: pull their vos.so folder, read every recipe (`.md`) and exemplar in it, find the declared seed, author work that follows them, check and render and judge locally, and push into that folder as attributed private versions. Recipes gate the work (a `STATUS:` line holds a seed until the owner signs off). | `npm i -D @vosjs/cli @vosso/vos-plugin`, a content key, a folder on vos.so |
